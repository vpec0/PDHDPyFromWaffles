    ...
    with gzip.open(fname, 'rb') as f :
        while True:
            ...
            try:
                ...
                AddData(hists_by_chan, bins_by_chan, *data)
            except EOFError:
                break
            counter += 1
    SaveToGZ(outpref+'all_hists.pkl.gz', (hists_by_chan, bins_by_chan))
...
def AddData(hists_by_chan, bins_by_chan, channels, wfms) :
    ...
    for chan,chwfms in zip(channels, wfms) :
        ...
        hist, xedges, yedges = np.histogram2d( x.flatten(), chwfms.to_numpy().flatten(), bins=[size,700], range=((0,size),(-500,200)) )
